...
def d_mean_square_loss_th(x, y, th, th0):
    ...
    return (1 / n) * np.sum(d_square_loss_th(x, y, th, th0), axis=1, keepdims=True)
...
def d_mean_square_loss_th0(x, y, th, th0):
    ...
    #Your code here 6
    n = x.shape[1]  # number of samples
    return (1 / n) * np.sum(d_square_loss_th0(x, y, th, th0))
...
############################################################
# Used in the test case for sgd, below
def num_grad(f):
    def df(x):
        g = np.zeros(x.shape)
        delta = 0.001
...
def sgdTest():
    ...
    def dJ(Xi, yi, w):
        ...
        return num_grad(f)(w)
    #Your code here 10
    #Insert code to call sgd on the above functions, and test the returned values
    w0 = np.zeros((X.shape[0], 1))
    step_size_fn = lambda i: 0.01 / (1 + 0.001 * i)
    max_iter = 100
    w, fs, ws = sgd(X, y, J, dJ, w0, step_size_fn, max_iter)
    print("Final weight vector:", w)
    print("Cost function values:", fs)
